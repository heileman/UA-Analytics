...
for college in colleges:
    ...
    departments = df.loc[df["College Name"] == college]["Department Name"].unique()
    ...
    for department in departments:
        ...
        degrees = df.loc[
            (df["College Name"] == college) & (df["Department Name"] == department)
        ]["Academic Career"].unique()
        ...
        for degree in degrees:
            ...
            ]["Major at Campus"].unique()

            # for each major
            for major in majors:
                program = major.split(" - ")[1]
                headcounts = df.loc[
                    (df["College Name"] == college)
                    & (df["Department Name"] == department)
                ...
                (numEmployee, averageSalary) = getData([program])

                # store demands for each level
                if numEmployee != 0 and averageSalary != 0:
                    degree_majors.append(program)
                    department_majors.append(program)
                    college_majors.append(program)
                    university_majors.append(program)
